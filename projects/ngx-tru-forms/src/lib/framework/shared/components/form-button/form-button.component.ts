...
@Component({
  ...
  template: ` <ng-container #button></ng-container> `,
})
export class FormButtonComponent implements OnInit, OnChanges {
  ...
  drawButton() {
    ...
    const componentFactory =
      this.componentFactoryResolver.resolveComponentFactory(
        this.jsonFormFieldsService[0].get("button")
      );
    ...
    (<ButtonComponent>componentRef.instance).handleButtonClick.subscribe(
      (event) => {
    ...
    (<ButtonComponent>componentRef.instance).color = this.submit
      ? "primary"
      : "";
  }
  ...
  getClass() {
    return this.cancel.length > 0
      ...
      : [
          "btn btn-primary button button--accept",
          this.submitClass || "",
          this.isFormValid ? "valid" : "invalid",
        ];
  }
