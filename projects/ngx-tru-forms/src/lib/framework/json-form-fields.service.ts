...
export class JsonFormFieldsService {
  ...
  has(type: string) {
    return (
      this.fieldTypes.hasOwnProperty(type) ||
  ...
  get(control): any {
    ...
    if (
      typeof control.schema.format !== "undefined" &&
      this.has(control.schema.format)
    ) {
      return types[control.schema.format];
    }

    if (
      typeof control.schema.enum !== "undefined" &&
      control.schema.type === "array"
    ) {
