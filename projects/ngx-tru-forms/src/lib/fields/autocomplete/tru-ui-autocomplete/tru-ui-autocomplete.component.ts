...
export class TruUiAutocompleteComponent
  ...
  implements OnInit
{
    ...
    if (this.schema.enumNames) {
      ...
      this.schema.enum.map((enm, i) => {
        if (Array.isArray(this.schema.enumNames[i])) {
          this.values.push({
            enum: enm,
            enumName: this.getTranslation(this.schema.enumNames[i]),
          });
        } else {
  ...
  typeaheadNoResults() {
    if (this.selectedValue !== "") {
      let searchValue = [];
      searchValue = this.schema.enumNames
        ? this.values.filter((en) => en.enumName === this.selectedValue)
        : this.values.filter((en) => en.enum === this.selectedValue);
      searchValue && searchValue?.length
        ? this.setValue(searchValue[0].enum)
        : this.setError();
    } else {
